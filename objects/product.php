...
class Product{
    ...
    function readAll($from_record_num, $records_per_page){
        $query = "SELECT id, name, description, price, category_id FROM " . $this->table_name . " ORDER BY id DESC LIMIT {$from_record_num}, {$records_per_page}";
        $stmt = $this->conn->prepare( $query );
        $stmt->execute();
    ...
    public function countAll_BySearch($search_term){
        // select query
        $query = "SELECT COUNT(*) as total_rows FROM " . $this->table_name . " p  WHERE p.name LIKE :some_value OR p.description LIKE :some_value";
        $stmt = $this->conn->prepare( $query );
        $search_term = "%{$search_term}%";
        $stmt->bindValue(':some_value', $search_term, PDO::PARAM_STR);
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
